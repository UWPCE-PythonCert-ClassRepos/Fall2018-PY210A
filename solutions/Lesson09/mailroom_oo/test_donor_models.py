...
def test_create_with_scalar():
    donor = Donor("Fred Flintstone", 500)
...
def test_find_or_create_donor():
    """ checks a donor that is there, but with odd case and spaces"""
    donor = sample_db.find_or_create_donor("jefF beZos ", 500)

    assert donor.name == "Jeff Bezos"
    assert donor.last_donation == 500


def test_find_or_create_donor_not_there():
    "test one that's not there"
    donor = sample_db.find_or_create_donor("Bob Jones", 500)
...
def test_gen_letter():
    ...
    donor = Donor("Fred Flintstone", [432.45, 65.45, 230.0])
