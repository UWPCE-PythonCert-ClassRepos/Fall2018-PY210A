...
def make_sum_donors(d_donors):
    sum_donors = [[
        k, [average_donations(v),
            total_donations(v),
            count_donations(v)]
    ] for (k, v) in d_donors.items()]
    return sum_donors
...
def make_report_strings(sum_donors):
    report_strings = [
        f'{donor[0]:<20} ${donor[1][1]:<15.2f} {donor[1][2]:<15} ${donor[1][0]:<15.2f}'
...
def make_total_donors(d_donors):
    return [[k, total_donations(v), make_filename(k)]
            for (k, v) in d_donors.items()]
...
def make_thank_you_list(total_donors):
    return [[thank_donor_msg(donor[0], donor[1]), donor[2]]
            for donor in total_donors]
...
def thank_donor_msg(donor_name, donation):
    return (
        f'Dear {donor_name}, Thank you for your donation of ${donation:.2f}.'
...
def thank_you():
    while True:
        ...
        if full_name.lower() == "list":
            ...
            send_file(
                thank_donor_msg(full_name, add_donation(full_name, donors)),
                make_filename(full_name))
            break


def report():
    print(make_headers_string())
    report_strings = make_report_strings(
        sorted(make_sum_donors(donors), key=sort_key, reverse=True))
