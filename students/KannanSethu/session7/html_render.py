...
class Element:
    ...
    def add_newlines(self):
        '''Loop through self.content and insert newlines if object in next index
         is not a string OR if last object in index is a string'''
        if any(isinstance(line, str) for line in self.content):
            ...
            for i in range(len(content_copy)-1):
                ...
                if isinstance(content_copy[i], object) and not isinstance(content_copy[i], str):
                    # print(f'Current object: {content_copy[i]}')
                    pass
    ...
    def render(self, out_file, **kwargs):
        ...
        if self.attrs:
            ...
            # print('Opening Indentation:', self.tag, indent)
        else:
        ...
        for line in self.content:
            if isinstance(line, str):
                ...
                # print('Found Object:', self, indent)
                line.render(out_file, indent=indent+4)
        out_file.write(f'{" "*indent}</{self.tag}>\n')
        # print('Closing Indentation:', self.tag, indent)
        return out_file
