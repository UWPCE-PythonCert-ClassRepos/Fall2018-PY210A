...
def test_anchor():
    ...
    # # just to be sure -- there should be a closing bracket to the opening tag
    assert file_contents[:-1].index(">") > file_contents.index('href="http://google.com"')
...
def test_unorderlist():
    ...
    # just to be sure -- there should be a closing bracket to the opening tag
    assert file_contents[:-1].index(">") > file_contents.index('id="TheList"')
    assert file_contents[:file_contents.index(">")].count(" ") == 2
    #assert False

def test_listitem_element():

    e = Li("first", id="TheList", style="line-height:200%")
    e.append("second")
    file_contents = render_result(e).strip()
...
def test_indent_contents():
    ...
    lines = file_contents.split("\n")
    print(lines)
    print(lines[1].startswith(Element.indent))
