...
def thank_you():
    ...
    while response != 'Q':
        ...
        response = get_user_response().title()
        if response == "List":
            print(print_donors())
        else:
...
def report_string():
    ...
    report_string_return = data_header(column_widths) + '\n'
    for j in output:
        report_string_return += data_print(j, column_widths) + '\n'
    report_string_return += '\n'
    return report_string_return
...
def main():
    ...
    while True:
        ...
        print('Send single thank you: "t"\n'
              ...
              'Quit: "q"')
        response = get_user_response()[:1].lower()
